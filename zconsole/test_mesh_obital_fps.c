...
int main(void)
{
	...
	while (qg_loop_dispatch())
	{
		...
		if (qg_begin_render(true))
		{
			...
			// 안내 메시ㅣㅈ
			qg_draw_text_format(0, QG_FGS(0), "평균 FPS: %.2f / %.2f", qg_get_afps(), qg_get_run_time());
			...
			// 시간
			dt.stamp = qn_now();
			qg_draw_text_format(0, QG_FGS(4), "실제 시간: %04d-%02d-%02d [%d] %02d:%02d:%02d.%03d", dt.year, dt.month, dt.day, dt.dow, dt.hour, dt.minute, dt.second, dt.millisecond);
			if (manual_constant)
			{
				int hh = (int)stime / 3600;
				int mm = (int)(stime - hh * 3600) / 60;
				int ss = (int)(stime - hh * 3600 - mm * 60);
				int ms = (int)((stime - (int)stime) * 1000);
				qg_draw_text_format(0, QG_FGS(5), "항성 시간: %02d:%02d:%02d.%03d", hh, mm, ss, ms);
			}
			else
			{
				dt.stamp = qn_qtc();
				qg_draw_text_format(0, QG_FGS(5), "내부 시간: %04d-%02d-%02d [%d] %02d:%02d:%02d.%03d", dt.year, dt.month, dt.day, dt.dow, dt.hour, dt.minute, dt.second, dt.millisecond);
			}
