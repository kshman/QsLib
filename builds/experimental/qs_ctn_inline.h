...
// 컨테이너 종류 인라인
#define QN_COMMON_INLINE_CTNR(TYPENAME, DATATYPE, PREFIX)	\
	/* @brief 컨테이너 제거 */\
	...
	/* @brief 컨테이너 갯수 */\
	FINLINE size_t QN_CONCAT(PREFIX, _count)(struct TYPENAME* self)	\
	{	\
		return self->COUNT;	\
	}	\
	/* @brief 컨테이너 데이터 포인터 */\
	...
	/* @brief 컨테이너 항목 얻기 */\
	FINLINE DATATYPE QN_CONCAT(PREFIX, _nth)(struct TYPENAME* self, size_t index)	\
	{	\
		return self->DATA[index];	\
	}	\
	/* @brief 컨테이너 항목 뒤에서 부터 얻기 */\
	FINLINE DATATYPE QN_CONCAT(PREFIX, _inv)(struct TYPENAME* self, size_t index)	\
	{	\
	...
	/* @brief 컨테이너가 비었나? */\
	FINLINE bool QN_CONCAT(PREFIX, _is_empty)(struct TYPENAME* self)	\
	{	\
		return self->COUNT == 0;	\
	}	\
	/* @brief 컨테이너 값이 있나? */\
	FINLINE bool QN_CONCAT(PREFIX, _is_have)(struct TYPENAME* self)	\
	{	\
		return self->COUNT != 0;	\
	}	\
	/* @brief 컨테이너 정렬하기 */\
	...
	FINLINE size_t QN_CONCAT(PREFIX, _contains)(struct TYPENAME* self, DATATYPE value)	\
	{	\
		DATATYPE* ptr = self->DATA;	\
		DATATYPE* end = ptr + self->COUNT;	\
		while (ptr < end)	\
	...
	FINLINE size_t QN_CONCAT(PREFIX, _find)(struct TYPENAME* self, bool(*compfunc)(void*, DATATYPE), void* context)	\
	{	\
		DATATYPE* ptr = self->DATA;	\
		DATATYPE* end = ptr + self->COUNT;	\
		while (ptr < end)	\
	...
	FINLINE size_t QN_CONCAT(PREFIX, _find_ptr)(struct TYPENAME* self, bool(*comfunc)(void*, void*), void* context)	\
	{	\
		DATATYPE* ptr = self->DATA;	\
		DATATYPE* end = ptr + self->COUNT;	\
		while (ptr < end)	\
	...
	FINLINE void QN_CONCAT(PREFIX, _foreach_1)(struct TYPENAME* self, void(*func)(DATATYPE))	\
	{	\
		DATATYPE* ptr = self->DATA;	\
		DATATYPE* end = ptr + self->COUNT;	\
		while (ptr < end)	\
	...
	/* @brief 컨테이너 foreach (콜백 사용) */\
	FINLINE void QN_CONCAT(PREFIX, _foreach_2)(struct TYPENAME* self, void(*func)(void*, DATATYPE), void* context)	\
	{	\
		DATATYPE* ptr = self->DATA;	\
		DATATYPE* end = ptr + self->COUNT;	\
		while (ptr < end)	\
	...
	FINLINE void QN_CONCAT(PREFIX, _foreach_ptr_1)(struct TYPENAME* self, void(*func)(void*))	\
	{	\
		DATATYPE* ptr = self->DATA;	\
		DATATYPE* end = ptr + self->COUNT;	\
		while (ptr < end)	\
	...
	/* @brief 컨테이너 포인터 foreach (콜백 사용) */\
	FINLINE void QN_CONCAT(PREFIX, _foreach_ptr_2)(struct TYPENAME* self, void(*func)(void*, void*), void* context)	\
	{	\
		DATATYPE* ptr = self->DATA;	\
		DATATYPE* end = ptr + self->COUNT;	\
		while (ptr < end)	\
...
/// @param PREFIX 함수 접두사
#define QN_DECL_INLINE_CTNR(TYPENAME, DATATYPE, PREFIX)	\
	typedef struct TYPENAME TYPENAME;	\
	struct TYPENAME	{ size_t COUNT; DATATYPE* DATA;	};	\
	QN_COMMON_INLINE_CTNR(TYPENAME, DATATYPE, PREFIX)	\
	/* @brief 컨테이너 초기화 */\
...
/// @param DATATYPE 데이터 타입
/// @param PREFIX 함수 접두사
#define QN_DECL_INLINE_ARRAY(TYPENAME, DATATYPE, PREFIX)	\
	typedef struct TYPENAME TYPENAME;	\
	struct TYPENAME	{ size_t COUNT, CAPA; DATATYPE* DATA; };	\
	QN_COMMON_INLINE_CTNR(TYPENAME, DATATYPE, PREFIX)	\
...
/// @param DATATYPE 데이터 타입
/// @param PREFIX 함수 접두사
#define QN_DECL_INLINE_LIST(TYPENAME, DATATYPE, PREFIX)	\
	...
	FINLINE size_t QN_CONCAT(PREFIX, _count)(TYPENAME *list)	\
	{	\
		return list->COUNT;	\
	}	\
	FINLINE bool QN_CONCAT(PREFIX, _is_have)(TYPENAME *list)	\
	{	\
		return list->COUNT != 0;	\
	}	\
	FINLINE bool QN_CONCAT(PREFIX, _is_empty)(TYPENAME *list)	\
	{	\
		return list->COUNT == 0;	\
	}	\
	FINLINE QN_CONCAT(TYPENAME, Node)* QN_CONCAT(PREFIX, _head)(TYPENAME *list)	\
	{	\
		return list->HEAD;	\
	}	\
	FINLINE QN_CONCAT(TYPENAME, Node)* QN_CONCAT(PREFIX, _tail)(TYPENAME *list)	\
	{	\
		return list->TAIL;	\
	}	\
	FINLINE DATATYPE QN_CONCAT(PREFIX, _head_data)(TYPENAME *list)	\
	{	\
		return list->HEAD->DATA;	\
	}	\
	FINLINE DATATYPE QN_CONCAT(PREFIX, _tail_data)(TYPENAME *list)	\
	{	\
		return list->TAIL->DATA;	\
	}	\
	FINLINE void QN_CONCAT(PREFIX, _init)(TYPENAME *list)	\
	{	\
		list->HEAD = list->TAIL = NULL;	\
		list->COUNT = 0;	\
	}	\
	FINLINE void QN_CONCAT(PREFIX, _dispose)(TYPENAME *list)	\
	{	\
	...
	FINLINE void QN_CONCAT(PREFIX, _dispose_callback)(TYPENAME *list, void(*callback)(void*, DATATYPE), void* context)	\
	{	\
		for (QN_CONCAT(TYPENAME, Node) *node = list->HEAD, *next; node; node = next)	\
		{	\
			next = node->NEXT;	\
	...
	FINLINE void QN_CONCAT(PREFIX, _dispose_ptr_callback)(TYPENAME *list, void(*callback)(void*, void*), void* context)	\
	{	\
		for (QN_CONCAT(TYPENAME, Node) *node = list->HEAD, *next; node; node = next)	\
		{	\
			next = node->NEXT;	\
	...
	FINLINE void QN_CONCAT(PREFIX, _clear)(TYPENAME *list)	\
	{	\
		QN_CONCAT(PREFIX, _dispose)(list);	\
		QN_CONCAT(PREFIX, _init)(list);	\
	}	\
	FINLINE void QN_CONCAT(PREFIX, _remove_node)(TYPENAME *list, QN_CONCAT(TYPENAME, Node) *node)	\
	{	\
		if (node == NULL)	\
			return;	\
		if (node->PREV)	\
	...
	FINLINE void QN_CONCAT(PREFIX, _remove_head)(TYPENAME *list)	\
	{	\
		QN_CONCAT(PREFIX, _remove_node)(list, list->HEAD);	\
	}	\
	FINLINE void QN_CONCAT(PREFIX, _remove_tail)(TYPENAME *list)	\
	{	\
		QN_CONCAT(PREFIX, _remove_node)(list, list->TAIL);	\
	}	\
	FINLINE void QN_CONCAT(PREFIX, _append)(TYPENAME *list, DATATYPE data)	\
	{	\
	...
	FINLINE void QN_CONCAT(PREFIX, _prepend)(TYPENAME *list, DATATYPE data)	\
	{	\
		QN_CONCAT(TYPENAME, Node) *node = qn_alloc_1(QN_CONCAT(TYPENAME, Node));	\
		node->DATA = data;	\
		node->NEXT = list->HEAD;	\
	...
	FINLINE void QN_CONCAT(PREFIX, _remove)(TYPENAME *list, DATATYPE data, bool once)	\
	{	\
		for (QN_CONCAT(TYPENAME, Node) *node = list->HEAD, *next; node; node = next)	\
		{	\
			next = node->NEXT;	\
	...
	FINLINE void QN_CONCAT(PREFIX, _remove_callback)(TYPENAME *list, DATATYPE data, bool once, bool(*compfunc)(void*, DATATYPE), void* context)	\
	{	\
		for (QN_CONCAT(TYPENAME, Node) *node = list->HEAD, *next; node; node = next)	\
		{	\
			next = node->NEXT;	\
	...
	FINLINE QN_CONCAT(TYPENAME, Node)* QN_CONCAT(PREFIX, _contains)(TYPENAME *list, DATATYPE data)	\
	{	\
	...
	FINLINE QN_CONCAT(TYPENAME, Node)* QN_CONCAT(PREFIX, _find)(TYPENAME *list, DATATYPE data, bool(*compfunc)(void*, DATATYPE), void* context)	\
	{	\
		for (QN_CONCAT(TYPENAME, Node) *node = list->HEAD; node; node = node->NEXT)	\
			if (compfunc(context, node->DATA))	\
				return node;	\
	...
	FINLINE QN_CONCAT(TYPENAME, Node)* QN_CONCAT(PREFIX, _find_ptr)(TYPENAME *list, DATATYPE data, bool(*compfunc)(void*, void*), void* context)	\
	{	\
		for (QN_CONCAT(TYPENAME, Node) *node = list->HEAD; node; node = node->NEXT)	\
			if (compfunc(context, &node->DATA))	\
				return node;	\
	...
	FINLINE void QN_CONCAT(PREFIX, _foreach_1)(TYPENAME *list, void(*callback)(DATATYPE))	\
	{	\
		for (QN_CONCAT(TYPENAME, Node) *node = list->HEAD; node; node = node->NEXT)	\
			callback(node->DATA);	\
	}	\
	FINLINE void QN_CONCAT(PREFIX, _foreach_2)(TYPENAME *list, void(*callback)(void*, DATATYPE), void* context)	\
	{	\
		for (QN_CONCAT(TYPENAME, Node) *node = list->HEAD; node; node = node->NEXT)	\
			callback(context, node->DATA);	\
	}	\
	FINLINE void QN_CONCAT(PREFIX, _foreach_ptr_1)(TYPENAME *list, void(*callback)(void*))	\
	{	\
		for (QN_CONCAT(TYPENAME, Node) *node = list->HEAD; node; node = node->NEXT)	\
			callback(&node->DATA);	\
	}	\
	FINLINE void QN_CONCAT(PREFIX, _foreach_ptr_2)(TYPENAME *list, void(*callback)(void*, void*), void* context)	\
	{	\
		for (QN_CONCAT(TYPENAME, Node) *node = list->HEAD; node; node = node->NEXT)	\
			callback(context, &node->DATA);	\
	}
...
/// @param PREFIX 함수 접두사
#define QN_DECL_INLINE_NODE_LIST(TYPENAME, NODETYPE, PREFIX)	\
	...
	FINLINE NODETYPE* QN_CONCAT(PREFIX, _head)(TYPENAME *list)	\
	{	\
		return list->HEAD;	\
	}	\
	FINLINE NODETYPE* QN_CONCAT(PREFIX, _tail)(TYPENAME *list)	\
	{	\
		return list->TAIL;	\
	}	\
	FINLINE void QN_CONCAT(PREFIX, _init)(TYPENAME *list)	\
	...
	FINLINE void QN_CONCAT(PREFIX, _dispose_callback)(TYPENAME *list, void(*callback)(void*))	\
	{	\
		for (NODETYPE *node = list->HEAD, *next; node; node = next)	\
		{	\
			next = node->NEXT;	\
	...
	FINLINE void QN_CONCAT(PREFIX, _reset)(TYPENAME *list)	\
	{	\
		QN_CONCAT(PREFIX, _init)(list);	\
	}	\
	FINLINE void QN_CONCAT(PREFIX, _clear)(TYPENAME *list)	\
	...
	FINLINE void QN_CONCAT(PREFIX, _clear_callback)(TYPENAME *list, void(*callback)(void*))	\
	{	\
		QN_CONCAT(PREFIX, _dispose_callback)(list, callback);	\
		QN_CONCAT(PREFIX, _init)(list);	\
	}	\
	FINLINE void QN_CONCAT(PREFIX, _remove)(TYPENAME *list, NODETYPE *node, bool freenode)	\
	{	\
		if (node == NULL)	\
			return;	\
		if (node->PREV)	\
	...
	FINLINE void QN_CONCAT(PREFIX, _remove_callback)(TYPENAME *list, NODETYPE *node, void(*callback)(void*))	\
	{	\
		if (node == NULL)	\
			return;	\
		if (node->PREV)	\
	...
	FINLINE void QN_CONCAT(PREFIX, _remove_head)(TYPENAME *list, bool freenode)	\
	{	\
		QN_CONCAT(PREFIX, _remove)(list, list->HEAD, freenode);	\
	}	\
	FINLINE void QN_CONCAT(PREFIX, _remove_head_callback)(TYPENAME *list, void(*callback)(void*))	\
	{	\
		QN_CONCAT(PREFIX, _remove_callback)(list, list->HEAD, callback);	\
	}	\
	FINLINE void QN_CONCAT(PREFIX, _remove_tail)(TYPENAME *list, bool freenode)	\
	{	\
		QN_CONCAT(PREFIX, _remove)(list, list->TAIL, freenode);	\
	}	\
	FINLINE void QN_CONCAT(PREFIX, _remove_tail_callback)(TYPENAME *list, void(*callback)(void*))	\
	{	\
		QN_CONCAT(PREFIX, _remove_callback)(list, list->TAIL, callback);	\
	}	\
	FINLINE void QN_CONCAT(PREFIX, _unlink)(TYPENAME *list, NODETYPE *node)	\
	{	\
		if (node == NULL)	\
			return;	\
		if (node->PREV)	\
	...
	FINLINE void QN_CONCAT(PREFIX, _append)(TYPENAME *list, NODETYPE* node)	\
	{	\
		node->NEXT = NULL;	\
		node->PREV = list->TAIL;	\
		if (list->TAIL)	\
	...
	FINLINE void QN_CONCAT(PREFIX, _prepend)(TYPENAME *list, NODETYPE* node)	\
	{	\
		node->NEXT = list->HEAD;	\
		node->PREV = NULL;	\
		if (list->HEAD)	\
	...
	FINLINE NODETYPE* QN_CONCAT(PREFIX, _contains)(TYPENAME *list, NODETYPE* dest)	\
	{	\
	...
	FINLINE NODETYPE* QN_CONCAT(PREFIX, _find)(TYPENAME *list, bool(*compfunc)(void*, void*), void* context)	\
	{	\
		for (NODETYPE *node = list->HEAD; node; node = node->NEXT)	\
	...
	FINLINE void QN_CONCAT(PREFIX, _foreach_1)(TYPENAME *list, void(*callback)(void*))	\
	{	\
	...
	FINLINE void QN_CONCAT(PREFIX, _foreach_2)(TYPENAME *list, void(*callback)(void*, void*), void* context)	\
	{	\
		for (NODETYPE *next, *node = list->HEAD; node; node = next)	\
		{	\
			next = node->NEXT;	\
