...
static void gl_vlo_dispose(QsGam* g);

qv_name(QsGam) vt_gl_vlo =
{
	.name = "GLVlo",
...
GlVlo* gl_vlo_allocator(QgRdh* rdh, int count, const QgLayoutInput* layouts)
{
	...
	return qs_init(self, GlVlo, &vt_gl_vlo);
}

//
static void gl_vlo_dispose(QsGam* g)
{
	GlVlo* self = qs_cast(g, GlVlo);
	for (size_t i = 0; i < QGLOS_MAX_VALUE; i++)
		if (self->es_elm[i])
...
static void gl_shd_dispose(QsGam* g);
...
GlShd* gl_shd_allocator(QgRdh* rdh, const char* name)
{
	...
	qn_val_if_fail(self, NULL);
#ifdef _MSC_VER
	qs_set_desc(self, gl_func.glCreateProgram());
#else
	qs_set_desc(self, GL_FUNC(glCreateProgram)());	// 이렇게 하면 LINT에 걸려버렷
#endif
	return qs_init(self, GlShd, &vt_gl_shd);
}

//
static void gl_shd_dispose(QsGam* g)
{
	GlShd* self = qs_cast(g, GlShd);
	const GLuint handle = qs_get_desc(self, GLuint);
...
static bool gl_shd_bind_shd(QgShd* g, QgShdType type, QgShd* shaderptr)
{
	...
	GlShd* self = qs_cast(g, GlShd);
	const GlShd* shader = qs_cast(shaderptr, GlShd);
	const GLuint handle = qs_get_desc(self, GLuint);
	bool ok = false;
...
bool gl_shd_link(QgShd* g)
{
	...
	// 링크
	const GLuint gl_program = qs_get_desc(self, GLuint);
...
void gl_shd_process_uniforms(GlShd* self)
{
	for (size_t i = 0; i < qn_ctnr_count(&self->uniforms); i++)
	{
		...
		if (u->base.role == QGSHR_AUTO)
			...
			(self->base.intr_func)(self->base.intr_data, &u->base, qs_cast(self, QgShd));
	}
}
...
static void gl_shd_auto_world_view_proj(const QgRdh* rdh, const GLint handle, const QgVarShader* v)
{
	qn_verify(v->type == QGSCT_FLOAT16); qn_verify(v->size == 1);
	QmMat4 m;
	qm_mat4_mul(&m, &rdh->tm.world, &rdh->tm.view_project);
...
static void gl_buf_dispose(QsGam* g);
...
GlBuf* gl_buf_allocator(QgRdh* rdh, QgBufType type, int count, int stride, const void* data)
{
	...
	return qs_init(self, GlBuf, &vt_gl_buf);
}

static void gl_buf_dispose(QsGam* g)
{
	GlBuf* self = qs_cast(g, GlBuf);

	if (self->lock_buf)
		qn_free(self->lock_buf);

	const GLuint gl_handle = qs_get_desc(self, GLuint);
...
static bool gl_buf_unmap(QgBuf* g)
{
	...
	gl_bind_buffer(GLBASE_RDH_INSTANCE, self->gl_type, qs_get_desc(self, GLuint));
