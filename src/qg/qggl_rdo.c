...
static void gl_dsm_dispose(QxGam* g);

qv_name(QxGam) vt_gl_dsm =
{
	.name = "GLDsm",
...
GlDsm* gl_dsm_allocator(QgRdh* rdh, const QgDepthStencilProp* prop)
{
	...
	return qx_init(self, GlDsm, &vt_gl_dsm);
}

static void gl_dsm_dispose(QxGam* g)
{
	GlDsm* self = qx_cast(g, GlDsm);
	qn_free(self);
}
...
static void gl_rsz_dispose(QxGam* g);

qv_name(QxGam) vt_gl_rsz =
{
	.name = "GLRsz",
...
GlRsz* gl_rsz_allocator(QgRdh* rdh, const QgRasterizerProp* prop)
{
	...
	return qx_init(self, GlRsz, &vt_gl_rsz);
}

static void gl_rsz_dispose(QxGam* g)
{
	GlRsz* self = qx_cast(g, GlRsz);
	qn_free(self);
}
