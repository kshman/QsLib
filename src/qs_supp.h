...
#define VAR_CHK_IF_NULL3(ctn,sec,item,ret)		QN_STMT_BEGIN{if ((ctn)->sec.item == NULL) { qn_debug_outputf(true, VAR_CHK_NAME, "%s: '%s.%s.%s' is null", __FUNCTION__, #ctn, #sec, #item); return ret; }}QN_STMT_END
#define VAR_CHK_IF_ZERO3(ctn,sec,item,ret)		QN_STMT_BEGIN{if ((size_t)((ctn)->sec.item) == 0) { qn_debug_outputf(true, VAR_CHK_NAME, "%s: '%s.%s.%s' is zero", __FUNCTION__, #ctn, #sec, #item); return ret; }}QN_STMT_END
#define VAR_CHK_IF_MAX3(ctn,sec,item,vmax,ret)	QN_STMT_BEGIN{if ((size_t)(ctn)->sec.item >= (vmax)) { qn_debug_outputf(true, VAR_CHK_NAME, "%s: invalid '%s.%s.%s' value: %d", __FUNCTION__, #ctn, #sec, #item, (ctn)->sec.item); return ret; }}QN_STMT_END
#define VAR_CHK_IF_MIN3(ctn,sec,item,vmin,ret)	QN_STMT_BEGIN{if ((ctn)->sec.item <= (vmin)) { qn_debug_outputf(true, VAR_CHK_NAME, "%s: invalid '%s.%s.%s' value: %d", __FUNCTION__, #ctn, #sec, #item, (ctn)->sec.item); return ret; }}QN_STMT_END
#else
