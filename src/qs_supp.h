...
#define VAR_CHK_IF_NEQ3(ctn,sec,item,value,ret)		QN_STMT_BEGIN{if (((ctn)->sec.item) == (value)) { qn_mesgf(true, VAR_CHK_NAME, "%s: '%s.%s.%s' must not %s", __FUNCTION__, #ctn, #sec, #item, #value); return ret; }}QN_STMT_END
#define VAR_CHK_IF_NEED3(ctn,sec,item,value,ret)	QN_STMT_BEGIN{if (((ctn)->sec.item) == 0) { qn_mesgf(true, VAR_CHK_NAME, "%s: '%s.%s.%s' is not %s", __FUNCTION__, #ctn, #sec, #item, #value); return ret; }}QN_STMT_END
